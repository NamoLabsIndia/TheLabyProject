package in.namolabs.laby.wear.mesh
...
import in.namolabs.laby.mesh.BluetoothConnectionManager
import in.namolabs.laby.mesh.BluetoothConnectionManagerDelegate
...
import in.namolabs.laby.storage.AppStateStore
import in.namolabs.laby.protocol.sync.GossipSyncManager
import in.namolabs.laby.util.AppConstants
...
class WearMeshService private constructor(private val context: Context) {
    ...
    var onPrivateMessage: ((in.namolabs.laby.protocol.model.BitchatMessage) -> Unit)? = null
    ...
    private fun handleMessageReceived(
        message: in.namolabs.laby.protocol.model.BitchatMessage
    ): Boolean = try {
        when {
            message.isPrivate -> {
     ...
     */
    fun sendFilePrivateEncrypted(recipientPeerID: String, file: in.namolabs.laby.protocol.model.BitchatFilePacket) {
        serviceScope.launch {
            ...
            val transferId = in.namolabs.laby.mesh.MeshPacketUtils.sha256Hex(
            ...
            while (System.currentTimeMillis() < prepDeadline) {
                when (val prep = meshCore.prepareFilePrivate(recipientPeerID, file, transferId, allowLegacyFallback = false)) {
                    is in.namolabs.laby.mesh.PrivateMediaPreparation.Ready -> {
                        prep.transfer.commit()
                        return@launch
                    }
                    in.namolabs.laby.mesh.PrivateMediaPreparation.AwaitingPeerState,
                    in.namolabs.laby.mesh.PrivateMediaPreparation.NeedsHandshake -> {
                        if (prep == in.namolabs.laby.mesh.PrivateMediaPreparation.NeedsHandshake) {
                            try { initiateNoiseHandshake(recipientPeerID) } catch (_: Exception) { }
                        }
                        delay(500)
                    }
    ...
    private fun prefs() = context.getSharedPreferences("laby_watch_prefs", Context.MODE_PRIVATE)
