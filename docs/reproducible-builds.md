...
```bash
git clone https://github.com/permissionlesstech/laby-android.git
cd laby-android
git checkout vX.Y.Z
...
```bash
apksigner verify --verbose --print-certs laby-android-universal.apk
```
...
tools/reproducible-builds/compare-archive-payloads.sh \
  .reproducible-build/local-vX.Y.Z/laby-android-universal-unsigned.apk \
...
gh attestation verify laby-android-universal-unsigned.apk \
  --repo permissionlesstech/laby-android
```
...
1. In Play Console, open **Test and release > App bundle explorer**, select the
   ...
   ```bash
   tools/reproducible-builds/compare-archive-payloads.sh \
     .reproducible-build/local-vX.Y.Z/laby-android-release-unsigned.aab \
...
4. Confirm package name `in.namolabs.laby`, version code, version name, and
...
5. Recreate Google's split-generation behavior from the reproduced AAB with the
   ...
   bundletool build-apks \
     --bundle=laby-android-release-unsigned.aab \
     --output=local.apks \
...
maintainer locally creates `laby-android-play-upload.aab` and
`laby-android-wear-play-upload.aab` from those exact files and uploads them
