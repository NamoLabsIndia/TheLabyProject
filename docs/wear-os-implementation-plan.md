...
- **Reference app**: this repository (`laby-android`) is a fully working, decentralized BLE mesh
  chat client. Single Gradle module `:app`, root package `in.namolabs.laby`, applicationId
  `in.namolabs.laby`. See `AGENTS.md` for the full architecture overview.
- **Goal**: a new `:wear` Gradle module (applicationId `in.namolabs.laby.wear`) — a standalone Wear OS
  app for the Pixel Watch. **Bluetooth mesh only**: global chat, Noise-encrypted direct messages,
  and receiving/displaying files & images. It must be a fully interoperable laby client: scan,
...
the laby protocol stack compiles unmodified.
...
`geohash/**`, `wifiaware/**`, `hotspot/**`, `features/voice/**`, `service/MeshForegroundService.kt`
(wear gets its own service), `LabyApplication.kt`, `MainActivity.kt`.
...
- Two phones running the Android laby app, also on ADB, for interop testing (used heavily from
...
"laby" wordmark (green `#32D74B`, Geist Mono, black background) correctly centered on the
...
references from shared code: `in.namolabs.laby.service.MeshServiceHolder` (BLE-toggle
interface, null) and `in.namolabs.laby.transport.wifiaware.WifiAwareController` (no-op).
...
**Success criteria**: the phone's laby app lists the watch as a connected peer and vice versa
...
- [ ] Chat timeline UI (`ScalingLazyColumn`, message bubbles per laby style, peer colors,
...
- [x] Wear debug-only `TestHookReceiver` (`wear/src/debug/`) mirroring the phone's command set:
  ...
  `clear_results` — broadcast action `in.namolabs.laby.wear.TEST_HOOK`, same JSON-result-file protocol
...
in `mesh_lab.py`: `WatchDevice` (package/hook/permissions/activity for `in.namolabs.laby.wear`),
...
adb -s <watch-serial> shell monkey -p in.namolabs.laby.wear -c android.intent.category.LAUNCHER 1
...
adb -s <watch-serial> shell am broadcast -a in.namolabs.laby.wear.TEST_HOOK \
  -n in.namolabs.laby.wear/.testhook.WearTestHookReceiver --es cmd state --es id s1
adb -s <watch-serial> shell run-as in.namolabs.laby.wear cat cache/testhook/results/s1.json
```
