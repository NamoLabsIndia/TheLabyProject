...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = in.namolabs.laby.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedLaby.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedLaby.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                val embedded = NostrEmbeddedLaby.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                in.namolabs.laby.ui.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is in.namolabs.laby.ui.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded Laby packet without recipient peer ID
                val embedded = NostrEmbeddedLaby.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            if (ContactIdentityResolver.isNoiseKeyHex(peerID)) {
                val noiseKey = ContactIdentityResolver.bytesFromHex(peerID) ?: return null
                val favoriteStatus = in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (ContactIdentityResolver.isMeshPeerId(peerID)) {
                val fallbackStatus = in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
