...
import in.namolabs.laby.storage.SeenMessageStore
import in.namolabs.laby.ui.ChatState
import in.namolabs.laby.ui.PrivateChatManager
import in.namolabs.laby.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: in.namolabs.laby.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (!content.startsWith("laby1:")) return@launch

                val base64Content = content.removePrefix("laby1:")
                ...
                if (packet.type != in.namolabs.laby.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                in.namolabs.laby.identity.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = in.namolabs.laby.ui.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        in.namolabs.laby.ui.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
