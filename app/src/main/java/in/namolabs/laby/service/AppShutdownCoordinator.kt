...
object AppShutdownCoordinator {
    ...
    fun requestFullShutdownAndKill(
        ...
        val job = scope.launch {
            ...
            try {
                val intent = android.content.Intent(in.namolabs.laby.util.AppConstants.UI.ACTION_FORCE_FINISH)
                    .setPackage(app.packageName)
                app.sendBroadcast(intent, in.namolabs.laby.util.AppConstants.UI.PERMISSION_FORCE_FINISH)
            } catch (_: Exception) { }

            // Stop mesh (best-effort)
            try { mesh?.stopServices() } catch (_: Exception) { }
            try { in.namolabs.laby.identity.nostr.NostrRelayManager.shared.disconnect() } catch (_: Exception) { }
            try { in.namolabs.laby.mesh.PowerManager.getInstance(app).shutdown() } catch (_: Exception) { }
            ...
            try { in.namolabs.laby.storage.AppStateStore.clear() } catch (_: Exception) { }
