package in.namolabs.laby.messaging

import android.content.Context
import android.util.Log
import in.namolabs.laby.messaging.FavoriteControlMessage
import in.namolabs.laby.mesh.MeshService
import in.namolabs.laby.protocol.model.ReadReceipt
import in.namolabs.laby.identity.nostr.NostrTransport
...
class MessageRouter private constructor(
            ...
            val instance = INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                        ...
                        try {
                            in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                        } catch (_: Exception) {}
                        INSTANCE = instance
                    }
                }
    ...
    private val favoriteListener = object: in.namolabs.laby.ui.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String): RouteResult {
        ...
        if (in.namolabs.laby.identity.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}… id=${messageID.take(8)}…")
            val recipientHex = in.namolabs.laby.identity.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                val sourceGeohash = in.namolabs.laby.identity.nostr.GeohashConversationRegistry.get(toPeerID)
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (in.namolabs.laby.identity.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = in.namolabs.laby.identity.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { in.namolabs.laby.identity.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
        val resolution = ContactDirectory.resolve(toPeerID)
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        ...
        val meshTarget = resolution.meshPeerID ?: toPeerID.takeIf { ContactIdentityResolver.isMeshPeerId(it) }
        if (meshTarget != null && mesh.getPeerInfo(meshTarget)?.isConnected == true && mesh.hasEstablishedSession(meshTarget)) {
            val myNpub = try { in.namolabs.laby.identity.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (ContactIdentityResolver.isNoiseKeyHex(target)) {
                ...
                val fav = in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else if (ContactIdentityResolver.isMeshPeerId(target)) {
                val fav = in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(target)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
