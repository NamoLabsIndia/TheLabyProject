package in.namolabs.laby.protocol
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            route = route,
            ttl = in.namolabs.laby.util.AppConstants.SYNC_TTL_HOPS, // Use fixed TTL=0 for signing to ensure relay compatibility
...
object BinaryProtocol {
    ...
    fun encode(packet: BitchatPacket, padding: Boolean = true): ByteArray? {
        try {
            val maxPayloadLength = in.namolabs.laby.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    private fun decodeCore(
        ...
        try {
            ...
            if (payloadLength > in.namolabs.laby.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${in.namolabs.laby.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
            ...
            val payload = if (isCompressed) {
                ...
                val maxExpandedSize = in.namolabs.laby.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
