package in.namolabs.laby.crypto.noise

import android.content.Context
import android.util.Log
import in.namolabs.laby.identity.SecureIdentityStateManager
import in.namolabs.laby.mesh.PeerFingerprintManager
import in.namolabs.laby.crypto.noise.southernstorm.protocol.Noise
...
class NoiseEncryptionService(private val context: Context) {
    ...
    companion object {
        ...
        private const val REKEY_TIME_LIMIT = in.namolabs.laby.util.AppConstants.Noise.REKEY_TIME_LIMIT_MS // 1 hour (same as iOS)
        private const val REKEY_MESSAGE_LIMIT = in.namolabs.laby.util.AppConstants.Noise.REKEY_MESSAGE_LIMIT_ENCRYPTION // 1k messages (matches iOS) (same as iOS)
    }
    ...
    private fun generateKeyPair(): Pair<ByteArray, ByteArray> {
        try {
            val dhState = in.namolabs.laby.crypto.noise.southernstorm.protocol.Noise.createDH("25519")
     ...
     */
    fun signPacket(packet: in.namolabs.laby.protocol.BitchatPacket): in.namolabs.laby.protocol.BitchatPacket? {
     ...
     */
    fun verifyPacketSignature(packet: in.namolabs.laby.protocol.BitchatPacket, publicKey: ByteArray): Boolean {
