package in.namolabs.laby.ui.debug
...
import in.namolabs.laby.services.meshgraph.MeshGraphService
...
import in.namolabs.laby.core.ui.component.sheet.BitchatSheetTopBar
import in.namolabs.laby.core.ui.component.sheet.BitchatSheetTitle
import in.namolabs.laby.util.DistributionInfoProvider
...
fun MeshTopologySection(
    ...
    val wifiAwareConnected by in.namolabs.laby.transport.wifiaware.WifiAwareController.connectedPeers.collectAsState()
                        ...
                        onClick = {
                            ...
                            clipboard?.setPrimaryClip(
                                ClipData.newPlainText(
                                    "Laby signing certificate SHA-256",
...
fun DebugSettingsSheet(
    ...
    val wifiAwareSupported by in.namolabs.laby.transport.wifiaware.WifiAwareController.supported.collectAsState()
    val wifiAwareAvailable by in.namolabs.laby.transport.wifiaware.WifiAwareController.available.collectAsState()
    val wifiAwareSupportStatus by in.namolabs.laby.transport.wifiaware.WifiAwareController.supportStatus.collectAsState()
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    in.namolabs.laby.ui.debug.ConnectedDevice(
                ...
                try {
                    val ctrl = in.namolabs.laby.transport.wifiaware.WifiAwareController
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        val running by in.namolabs.laby.transport.wifiaware.WifiAwareController.running.collectAsState()
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            AssistChip(
                                onClick = { in.namolabs.laby.transport.wifiaware.WifiAwareController.getService()?.sendBroadcastAnnounce() },
                                enabled = running,
                                label = { Text("Announce") }
                            )
                        }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { in.namolabs.laby.protocol.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { in.namolabs.laby.protocol.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
