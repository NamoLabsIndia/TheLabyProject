...
import in.namolabs.laby.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { in.namolabs.laby.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }

        // Clean up channel members who disconnected
        channelManager.cleanupDisconnectedMembers(mergedPeers, getMyPeerID())

        runCatching { in.namolabs.laby.storage.AppStateStore.canonicalizePrivateChats() }
        ...
        state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
            ...
            if (canonical != currentPeer) {
                in.namolabs.laby.services.ConversationAliasResolver.unifyChatsIntoPeer(
                    state = state,
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        in.namolabs.laby.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
        ...
        if (shouldSendReadReceipt) {
            ...
            try {
                val meshPeerID = ContactDirectory.resolve(senderConversationID).meshPeerID
                    ?: senderPeerID.takeIf {
                        in.namolabs.laby.services.ContactIdentityResolver.isMeshPeerId(it)
                    }
                if (meshPeerID != null &&
     ...
     */
    fun getPeerInfo(peerID: String): in.namolabs.laby.mesh.PeerInfo? {
