...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = in.namolabs.laby.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = in.namolabs.laby.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    ...
    fun addMessage(message: BitchatMessage) {
        ...
        try { in.namolabs.laby.storage.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: BitchatMessage) {
        ...
        try { in.namolabs.laby.storage.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is in.namolabs.laby.ui.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: BitchatMessage) {
        ...
        val accepted = try {
            in.namolabs.laby.storage.AppStateStore.addPrivateMessage(
                conversationID,
                message
            )
        } catch (_: Exception) {
    ...
    suspend fun addPrivateMessageDurably(
        ...
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            in.namolabs.laby.storage.AppStateStore.addPrivateMessageDurably(
    ...
    fun clearPrivateMessages(peerID: String) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        in.namolabs.laby.storage.AppStateStore.deletePrivateConversation(conversationID)
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { in.namolabs.laby.storage.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
    ...
    fun removeMessageById(messageID: String) {
        ...
        run {
            ...
            if (changed) {
                ...
                in.namolabs.laby.storage.AppStateStore.removePrivateMessage(messageID)
            }
        }
        // Channels
        run {
