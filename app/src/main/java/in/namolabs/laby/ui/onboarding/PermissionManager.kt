package in.namolabs.laby.ui.onboarding
...
/**
 * Centralized permission management for laby app
...
class PermissionManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "laby_permissions"
    ...
    private fun shouldRequireWifiAwarePermission(): Boolean {
        ...
        val enabled = try {
            in.namolabs.laby.ui.debug.DebugPreferenceManager.getWifiAwareEnabled(false)
        } catch (_: Exception) {
            false
        }
        ...
        return try {
            in.namolabs.laby.transport.wifiaware.WifiAwareSupport.isSupported(context)
        } catch (_: Exception) {
            false
        }
    }
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover laby users via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow laby to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby laby users via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "laby needs this to scan for nearby devices"
            )
        )
        ...
        if (shouldRequireWifiAwarePermission()) {
            ...
            categories.add(
                PermissionCategory(
                    ...
                    description = "Enable Wi‑Fi Aware to discover and connect to nearby laby users over Wi‑Fi.",
                    permissions = wifiAwarePermissions,
                    isGranted = wifiAwarePermissions.all { isPermissionGranted(it) },
                    systemDescription = "Allow laby to discover nearby Wi‑Fi devices"
                )
            )
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    systemDescription = "Allow laby to send you notifications"
                )
            )
        }
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure laby runs reliably in the background and maintains mesh network connections",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow laby to run without battery restrictions"
                )
            )
        }
