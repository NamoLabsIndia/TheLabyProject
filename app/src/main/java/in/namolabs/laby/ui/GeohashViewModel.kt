...
import in.namolabs.laby.ui.geohash.GeohashNostrPrivacyPolicy
import in.namolabs.laby.ui.geohash.LiveLocationPrivacyGate
import in.namolabs.laby.identity.nostr.GeohashMessageHandler
import in.namolabs.laby.identity.nostr.GeohashRepository
import in.namolabs.laby.identity.nostr.NostrBackgroundRuntime
import in.namolabs.laby.identity.nostr.NostrIdentityBridge
import in.namolabs.laby.identity.nostr.NostrProtocol
import in.namolabs.laby.identity.nostr.NostrRelayManager
import in.namolabs.laby.identity.nostr.NostrSubscriptionManager
import in.namolabs.laby.identity.nostr.PoWPreferenceManager
import in.namolabs.laby.identity.nostr.GeohashAliasRegistry
...
class GeohashViewModel(
    ...
    private var locationChannelManager: in.namolabs.laby.ui.geohash.LocationChannelManager? = null
    ...
    fun initialize() {
        ...
        try {
            locationChannelManager = in.namolabs.laby.ui.geohash.LocationChannelManager.getInstance(getApplication())
            ...
            state.setSelectedLocationChannel(in.namolabs.laby.ui.geohash.ChannelID.Mesh)
    ...
    fun panicReset() {
        ...
        try { in.namolabs.laby.identity.nostr.NostrRelayManager.getInstance(getApplication()).clearAllOnPanic() } catch (_: Exception) {}
        try { in.namolabs.laby.identity.nostr.LocationNotesManager.getInstance().stop() } catch (_: Exception) {}
    }

    fun sendGeohashMessage(content: String, channel: in.namolabs.laby.ui.geohash.GeohashChannel, myPeerID: String, nickname: String?) {
        viewModelScope.launch {
            try {
                ...
                val localMsg = in.namolabs.laby.protocol.model.BitchatMessage(
                    id = tempId,
    ...
    fun startGeohashDM(pubkeyHex: String, onStartPrivateChat: (String) -> Unit) {
        ...
        val gh = (current as? in.namolabs.laby.ui.geohash.ChannelID.Location)?.channel?.geohash
        if (!gh.isNullOrEmpty()) {
    ...
    fun blockUserInGeohash(targetNickname: String) {
        ...
        if (pubkey != null) {
            ...
            val sysMsg = in.namolabs.laby.protocol.model.BitchatMessage(
                sender = "system",
    ...
    private fun switchLocationChannel(channel: in.namolabs.laby.ui.geohash.ChannelID?) {
        ...
        when (channel) {
            is in.namolabs.laby.ui.geohash.ChannelID.Mesh -> {
            ...
            is in.namolabs.laby.ui.geohash.ChannelID.Location -> {
                if (locationChannelManager?.canUseSelectedLocationChannel(channel.channel) != true) {
                    ...
                    switchLocationChannel(in.namolabs.laby.ui.geohash.ChannelID.Mesh)
                    return
                }
    ...
    override fun onStart(owner: LifecycleOwner) {
        ...
        val selectedLocation = selected as? in.namolabs.laby.ui.geohash.ChannelID.Location
