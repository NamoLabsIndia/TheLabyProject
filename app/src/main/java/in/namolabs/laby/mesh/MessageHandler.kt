...
import in.namolabs.laby.messaging.FavoriteControlMessage
...
import in.namolabs.laby.protocol.sync.PacketIdUtil
import in.namolabs.laby.util.toHexString
import in.namolabs.laby.messaging.LiveVoiceManager
import in.namolabs.laby.messaging.LiveVoiceScope
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket): Boolean {
        ...
        try {
            ...
            val noisePayload = in.namolabs.laby.protocol.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                in.namolabs.laby.protocol.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = in.namolabs.laby.protocol.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = in.namolabs.laby.protocol.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "Encrypted file from $peerID: ${file.fileSize} bytes")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = in.namolabs.laby.ui.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = in.namolabs.laby.ui.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.VOICE_FRAME -> {
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.PEER_STATE -> {
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.DELIVERED -> {
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                in.namolabs.laby.protocol.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                    delegate?.onVerifyChallengeReceived(peerID, noisePayload.data, packet.timestamp.toLong())
                }
                in.namolabs.laby.protocol.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = in.namolabs.laby.protocol.model.NoisePayload(
                type = in.namolabs.laby.protocol.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounceWithResult(routed: RoutedPacket): AnnounceHandlingResult {
        ...
        try {
            val neighborsOrNull = in.namolabs.laby.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = in.namolabs.laby.protocol.MessageType.fromValue(packet.type) == in.namolabs.laby.protocol.MessageType.FILE_TRANSFER
            val file = in.namolabs.laby.protocol.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
    ...
    private suspend fun handlePrivateMessage(packet: BitchatPacket, peerID: String) {
        try {
            val isFileTransfer = in.namolabs.laby.protocol.MessageType.fromValue(packet.type) ==
                in.namolabs.laby.protocol.MessageType.FILE_TRANSFER
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, control.isFavorite)
                if (control.npub != null) {
                    in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, control.npub)
                    in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, control.npub)
                }

                val rel = in.namolabs.laby.ui.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val sys = in.namolabs.laby.protocol.model.BitchatMessage(
                    sender = "system",
                    content = notice,
                ...
                try {
                    val conversationID = in.namolabs.laby.services.ContactDirectory
                        .canonicalConversationId(fromPeerID)
                    val sysPrivate = in.namolabs.laby.protocol.model.BitchatMessage(
                        sender = "system",
                        content = notice,
                        timestamp = java.util.Date(),
                        isRelay = false,
...
interface MessageHandlerDelegate {
    ...
    fun updatePeerInfoFromVerifiedAnnouncement(
        ...
        isVerified: Boolean,
        capabilities: in.namolabs.laby.protocol.model.PeerCapabilities? = null
    ): Boolean
