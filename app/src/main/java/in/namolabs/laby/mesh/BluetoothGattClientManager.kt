...
import in.namolabs.laby.ui.debug.DebugSettingsManager
import in.namolabs.laby.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    private fun isClientRoleEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { in.namolabs.laby.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val dbg = try { in.namolabs.laby.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        Log.d(TAG, "Connecting to laby device: $deviceAddress (peerID: $peerID)")
