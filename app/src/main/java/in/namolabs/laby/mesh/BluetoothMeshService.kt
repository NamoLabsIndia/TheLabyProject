...
import in.namolabs.laby.crypto.EncryptionService
import in.namolabs.laby.protocol.model.BitchatMessage
import in.namolabs.laby.protocol.model.AuthenticatedPeerState
import in.namolabs.laby.protocol.model.PeerCapabilities
import in.namolabs.laby.protocol.MessagePadding
import in.namolabs.laby.protocol.model.RoutedPacket
import in.namolabs.laby.protocol.model.IdentityAnnouncement
import in.namolabs.laby.protocol.model.NoisePayload
...
import in.namolabs.laby.protocol.SpecialRecipients
import in.namolabs.laby.protocol.model.RequestSyncPacket
import in.namolabs.laby.protocol.sync.GossipSyncManager
import in.namolabs.laby.util.toHexString
import in.namolabs.laby.services.VerificationService
import in.namolabs.laby.service.TransportBridgeService
...
class BluetoothMeshService(private val context: Context) : TransportBridgeService.TransportLayer {
    ...
    companion object {
        ...
        private val MAX_TTL: UByte = in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS
        private const val PEER_DISCONNECT_GRACE_MS = in.namolabs.laby.util.AppConstants.Mesh.PEER_DISCONNECT_GRACE_MS
    }
    ...
    private val privateMediaPreparer by lazy {
        PrivateMediaTransferPreparer(
            ...
            encrypt = { plaintext, peerID, authenticatedSession ->
                ...
                } catch (_: in.namolabs.laby.crypto.noise.NoiseSessionError.SessionGenerationChanged) {
                    PrivateMediaEncryptionResult.GenerationChanged
                } catch (_: in.namolabs.laby.crypto.noise.NoiseSessionError.SessionNotFound) {
                    PrivateMediaEncryptionResult.GenerationChanged
                } catch (_: in.namolabs.laby.crypto.noise.NoiseSessionError.SessionNotEstablished) {
                    PrivateMediaEncryptionResult.GenerationChanged
                } catch (_: Exception) {
    ...
    private val serviceNotificationManager = in.namolabs.laby.ui.NotificationManager(
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    in.namolabs.laby.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    in.namolabs.laby.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    in.namolabs.laby.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
                } catch (_: Exception) { 0.01 }
            }
        )

        in.namolabs.laby.service.MeshServiceHolder.setGossipManager(gossipSyncManager) { packet ->
    ...
    private fun isBleTransportEnabled(): Boolean {
        return try {
            in.namolabs.laby.ui.debug.DebugSettingsManager.getInstance().bleEnabled.value
        } catch (_: Exception) {
            try { in.namolabs.laby.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        }
    }
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { in.namolabs.laby.storage.AppStateStore.setTransportPeers("BLE", peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { in.namolabs.laby.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(peerID: String, nickname: String, noisePublicKey: ByteArray, signingPublicKey: ByteArray, isVerified: Boolean, capabilities: in.namolabs.laby.protocol.model.PeerCapabilities?): Boolean {
            ...
            override fun decryptFromPeer(
                ...
                senderPeerID: String
            ): in.namolabs.laby.crypto.noise.NoiseDecryptionResult? {
            ...
            override fun onAuthenticatedPeerStateReceived(
                ...
                authenticatedSession: in.namolabs.laby.crypto.noise.AuthenticatedNoiseSession
            ) {
            ...
            override fun onMessageReceived(message: BitchatMessage) {
                ...
                if (
                    !in.namolabs.laby.services.IncomingMessageAdmission
                ...
                if (delegate == null && message.isPrivate && message.sender != "system") {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = in.namolabs.laby.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                ...
                try {
                    in.namolabs.laby.storage.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        in.namolabs.laby.protocol.model.DeliveryStatus.Delivered(peerID, Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    in.namolabs.laby.storage.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        in.namolabs.laby.protocol.model.DeliveryStatus.Read(peerID, Date())
                    )
                } catch (_: Exception) { }
        ...
        override fun onPacketReceived(
            ...
            try {
                in.namolabs.laby.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    in.namolabs.laby.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun startServices() {
        ...
        if (!isBleTransportEnabled()) {
            ...
            in.namolabs.laby.service.MeshServiceHolder.stopSharedGossip("BLE")
            try { in.namolabs.laby.storage.AppStateStore.clearTransportPeers("BLE") } catch (_: Exception) { }
            try { in.namolabs.laby.storage.AppStateStore.clearTransportDirectPeers("BLE") } catch (_: Exception) { }
            return
        }
        if (terminated) {
        ...
        if (connectionManager.startServices()) {
            ...
            in.namolabs.laby.service.MeshServiceHolder.startSharedGossip("BLE")
        } else {
    ...
    fun sendFileBroadcast(file: in.namolabs.laby.protocol.model.BitchatFilePacket) {
        try {
    ...
    fun sendFilePrivate(recipientPeerID: String, file: in.namolabs.laby.protocol.model.BitchatFilePacket) {
    ...
    fun prepareFilePrivate(
        ...
        file: in.namolabs.laby.protocol.model.BitchatFilePacket,
        transferId: String,
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = in.namolabs.laby.protocol.model.PrivateMessagePacket(
                    ...
                    val messagePayload = in.namolabs.laby.protocol.model.NoisePayload(
                        type = in.namolabs.laby.protocol.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { in.namolabs.laby.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = in.namolabs.laby.identity.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(in.namolabs.laby.protocol.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Create read receipt payload using NoisePayloadType exactly like iOS
                val readReceiptPayload = in.namolabs.laby.protocol.model.NoisePayload(
                    type = in.namolabs.laby.protocol.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BitchatPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
                ...
                readReceiptRetrySender.enqueue(
                    ...
                    onComplete = { accepted ->
                        if (accepted) {
                            try {
                                in.namolabs.laby.storage.SeenMessageStore
    ...
    fun sendBroadcastAnnounce() {
        serviceScope.launch {
            val nickname = try { in.namolabs.laby.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = in.namolabs.laby.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    in.namolabs.laby.services.meshgraph.MeshGraphService.getInstance()
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { in.namolabs.laby.storage.AppStateStore.setTransportDirectPeers("BLE", direct) } catch (_: Exception) { }
            val union = try {
                in.namolabs.laby.storage.AppStateStore.getDirectPeers().ifEmpty { direct }
     ...
     */
    fun getSessionState(peerID: String): in.namolabs.laby.crypto.noise.NoiseSession.NoiseSessionState {
    ...
    private fun applyRouteIfAvailable(packet: BitchatPacket): BitchatPacket {
        return try {
            ...
            if (recipient != null && !recipient.contentEquals(SpecialRecipients.BROADCAST)) {
                ...
                val path = in.namolabs.laby.services.meshgraph.RoutePlanner.shortestPath(
                    myPeerID,
                    destination
                )
