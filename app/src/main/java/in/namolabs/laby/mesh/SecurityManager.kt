...
import in.namolabs.laby.crypto.noise.AuthenticatedNoiseSession
import in.namolabs.laby.crypto.noise.NoiseDecryptionResult
...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = in.namolabs.laby.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = in.namolabs.laby.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = in.namolabs.laby.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = in.namolabs.laby.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
        private const val KEY_EXCHANGE_DEDUP_TIMEOUT = in.namolabs.laby.util.AppConstants.Security.KEY_EXCHANGE_DEDUP_TIMEOUT_MS
    }
    ...
    fun validatePacket(packet: BitchatPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS
    ...
    suspend fun handleNoiseHandshake(routed: RoutedPacket): Boolean {
        ...
        try {
            ...
            if (result.establishedNow) {
                ...
                val isDirectIngress = packet.ttl == in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS
