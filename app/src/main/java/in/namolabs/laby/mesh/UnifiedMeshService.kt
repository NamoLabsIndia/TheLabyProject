...
class UnifiedMeshService(
    ...
    override fun sendFavoriteNotification(peerID: String, isFavorite: Boolean) {
        val myNpub = try {
            in.namolabs.laby.identity.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub
        } catch (_: Exception) {
            null
        }
    ...
    private fun mergedPeerIDs(): List<String> {
        ...
        try { ids.addAll(in.namolabs.laby.storage.AppStateStore.peers.value) } catch (_: Exception) { }
