...
/**
 * Shared transport send wrapper that applies laby packet fragmentation and
...
class FragmentingPacketSender(
    ...
    private fun packetsForTransport(routed: RoutedPacket): List<BitchatPacket>? {
        routed.preparedPackets?.let { prepared ->
            if (prepared.isEmpty() ||
                prepared.size > in.namolabs.laby.util.AppConstants.Fragmentation.MAX_FRAGMENTS_PER_ID) {
