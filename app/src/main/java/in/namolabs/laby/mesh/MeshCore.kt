...
class MeshCore(
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { in.namolabs.laby.storage.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(
                ...
                isVerified: Boolean,
                capabilities: in.namolabs.laby.protocol.model.PeerCapabilities?
            ): Boolean {
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                try {
                    in.namolabs.laby.storage.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        in.namolabs.laby.protocol.model.DeliveryStatus.Delivered(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    in.namolabs.laby.storage.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        in.namolabs.laby.protocol.model.DeliveryStatus.Read(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = in.namolabs.laby.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = in.namolabs.laby.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += in.namolabs.laby.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { in.namolabs.laby.storage.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                in.namolabs.laby.storage.AppStateStore.getDirectPeers().ifEmpty { localDirect }
