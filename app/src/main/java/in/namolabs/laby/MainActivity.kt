...
import in.namolabs.laby.ui.geohash.LocationChannelManager
import in.namolabs.laby.ui.onboarding.BluetoothCheckScreen
import in.namolabs.laby.ui.onboarding.BluetoothStatus
import in.namolabs.laby.ui.onboarding.BluetoothStatusManager
import in.namolabs.laby.ui.onboarding.BatteryOptimizationManager
import in.namolabs.laby.ui.onboarding.BatteryOptimizationPreferenceManager
import in.namolabs.laby.ui.onboarding.BatteryOptimizationScreen
import in.namolabs.laby.ui.onboarding.BatteryOptimizationStatus
import in.namolabs.laby.ui.onboarding.BackgroundLocationPermissionScreen
import in.namolabs.laby.ui.onboarding.InitializationErrorScreen
import in.namolabs.laby.ui.onboarding.InitializingScreen
import in.namolabs.laby.ui.onboarding.LocationCheckScreen
import in.namolabs.laby.ui.onboarding.LocationStatus
import in.namolabs.laby.ui.onboarding.LocationStatusManager
import in.namolabs.laby.ui.onboarding.OnboardingCoordinator
import in.namolabs.laby.ui.onboarding.OnboardingState
import in.namolabs.laby.ui.onboarding.PermissionExplanationScreen
import in.namolabs.laby.ui.onboarding.PermissionManager
import in.namolabs.laby.ui.ChatScreen
import in.namolabs.laby.ui.ChatViewModel
import in.namolabs.laby.ui.OrientationAwareActivity
import in.namolabs.laby.ui.theme.BitchatTheme
import in.namolabs.laby.transport.wifiaware.WifiAwareController
import in.namolabs.laby.identity.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == in.namolabs.laby.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(in.namolabs.laby.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                in.namolabs.laby.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        in.namolabs.laby.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        meshService = in.namolabs.laby.service.MeshServiceHolder.getOrCreate(applicationContext)
        unifiedMeshService = in.namolabs.laby.service.MeshServiceHolder.getUnifiedOrCreate(applicationContext)
    ...
    private fun checkBluetoothAndProceed() {
        ...
        val bleRequired = try { in.namolabs.laby.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        if (!bleRequired) {
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !in.namolabs.laby.ui.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun handleLocationEnabled() {
        ...
        in.namolabs.laby.transport.wifiaware.WifiAwareController.startIfPossible()
        checkBatteryOptimizationAndProceed()
    }
    ...
    private fun handleOnboardingComplete() {
        ...
        val bleRequired2 = try { in.namolabs.laby.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        when {
    ...
    private fun startMeshForegroundServiceBestEffort() {
        ...
        try {
            in.namolabs.laby.service.MeshForegroundService.start(applicationContext)
    ...
    private fun initializeApp() {
        lifecycleScope.launch {
            try {
                ...
                in.namolabs.laby.identity.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            in.namolabs.laby.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            in.namolabs.laby.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(in.namolabs.laby.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(in.namolabs.laby.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(in.namolabs.laby.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.BLOCK
                        6 -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.CITY
                        4 -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.PROVINCE
                        2 -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.REGION
                        else -> in.namolabs.laby.ui.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = in.namolabs.laby.ui.geohash.GeohashChannel(level, geohash)
                    val channelId = in.namolabs.laby.ui.geohash.ChannelID.Location(geohashChannel)
    ...
    private fun handleVerificationIntent(intent: Intent) {
        ...
        if (uri.scheme != "laby" || uri.host != "verify") return
