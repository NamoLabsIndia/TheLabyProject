...
import in.namolabs.laby.crypto.noise.NoiseHandshakeProcessingResult
import in.namolabs.laby.crypto.noise.NoisePeerIdentity
import in.namolabs.laby.crypto.noise.NoiseSessionError
...
class SecurityManagerTest {
    ...
    fun `validatePacket - handles ANNOUNCE duplicates correctly`() {
        ...
        val packet1 = BitchatPacket(
            type = MessageType.ANNOUNCE.value,
            ttl = in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS, // 7u
            senderID = unknownPeerID,
            payload = payload
        )
        ...
        val packet2 = packet1.copy(ttl = (in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS - 1u).toUByte())
        ...
        val packet3 = packet1.copy(ttl = in.namolabs.laby.util.AppConstants.MESSAGE_TTL_HOPS)
    ...
    fun `handshake dedup entries expire by time and allow delayed retry`() = runBlocking {
        ...
        securityManager.cleanupOldData(
            System.currentTimeMillis() +
                in.namolabs.laby.util.AppConstants.Security.KEY_EXCHANGE_DEDUP_TIMEOUT_MS + 1_000
        )
