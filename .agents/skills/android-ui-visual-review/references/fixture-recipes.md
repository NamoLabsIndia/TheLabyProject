...
- `app/src/debug/java/in/namolabs/laby/testhook/TestHookReceiver.kt`
- `app/src/debug/java/in/namolabs/laby/testhook/TestHookDriver.kt`
...
adb -s "$ANDROID_REVIEW_SERIAL" shell am broadcast \
  -n in.namolabs.laby/in.namolabs.laby.testhook.TestHookReceiver \
  -a in.namolabs.laby.TEST_HOOK \
  --es cmd "<command>" \
...
```sh
adb -s "$ANDROID_REVIEW_SERIAL" shell run-as in.namolabs.laby \
...
private fun resetNicknameFixture(context: Context): JSONObject {
    context.getSharedPreferences("laby_prefs", Context.MODE_PRIVATE)
        .edit()
        .remove("nickname")
        .commit()
