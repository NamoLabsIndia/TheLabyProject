...
5. Inspect
   `app/src/debug/java/in/namolabs/laby/testhook/TestHookDriver.kt` before
...
- Before `setup`, `identity_reset`, or `all`, confirm that the selected devices
  may have laby app data cleared. `identity_reset` clears device B even when
